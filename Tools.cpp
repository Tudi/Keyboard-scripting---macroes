...
//	printf( "Class name %s\n", ClassName );
	if( GlobalStore.PrintKeysPressed == true && strlen( WindowTitle ) > 1 )
	{
		...
		char ParentWindowTitle[ DEFAULT_BUFLEN ];
		strcpy_s( ParentWindowTitle, DEFAULT_BUFLEN, "" );
		HWND hwndParent = GetParent( hwnd );
		if( hwndParent )
		{
			GetWindowText( hwndParent, ParentWindowTitle, sizeof( ParentWindowTitle ) );
			if( strlen( ParentWindowTitle ) > 1 )
				printf( "\t parent window name %s\n", ParentWindowTitle );
		}
	}
